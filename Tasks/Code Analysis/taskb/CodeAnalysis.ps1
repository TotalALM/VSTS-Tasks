...
(
    ...
    [String] [Parameter(Mandatory = $false)] $includeSummary,#whether or not to include summary to output
	[String] [Parameter(Mandatory = $false)] $logging,
...
$FxCopEXE = "$FxCopInstallationPath\FxCop\FxCopCmd.exe"

#Compile files to run analysis
$fileList.Split(",") | foreach {
    Write-Host "Include file: $buildDirectory"
